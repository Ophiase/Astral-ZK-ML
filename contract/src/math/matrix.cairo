...
pub impl MatrixAdd of Add<Matrix> {
    fn add(lhs: Matrix, rhs: Matrix) -> Matrix {
        assert!(lhs.shape() == rhs.shape(), "add error: different size");
        ...
        loop {
            ...
            result.append(left_content + right_content);
            i += 1;
        };
...
pub impl MatrixBasics of IMatrixBasics {
    ...
    fn apply<T, +Copy<T>, +Drop<T>, +IComponentLambda<T, WFloat, WFloat>>(
    ...
    fn apply_extern<
        T, U, +Drop<U>, +Copy<T>, +Drop<T>, +Destruct<T>, +IComponentLambda<T, WFloat, U>
    >(
        self: @Matrix, lambda: T
    ) -> Span<Span<U>> {
        let dimX = self.dimX();
    ...
    fn vertical_split(self: @Matrix, where: usize) -> (Matrix, Matrix) {
        ...
        (Matrix { content: left.span() }, Matrix { content: right.span() })
    }
