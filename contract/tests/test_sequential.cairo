...
use astral_zkml::math::ml::{
    Sequential, SequentialBasics, ActivationFunction, DEFAULT_SGD, SGD, LossFunctionType,
...
fn build_neural_network() {
    ...
    let mut model = SequentialBasics::init(layers, DEFAULT_SGD, Option::None);
...
fn verify_network_() {
    ...
    let layers: Span<DenseLayer> = array![
        ...
        DenseLayerBasics::add(output_shape: 10, activation_function: ActivationFunction::ReLU,),
        DenseLayerBasics::add(output_shape: 3, activation_function: ActivationFunction::ReLU,),
    ]
        .span();

    let mut model = SequentialBasics::init(
        layers, SGD { learning_rate: DECIMAL_WFLOAT, loss: LossFunctionType::MSE }, Option::None
    );
